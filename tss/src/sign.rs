...
pub async fn run(
    ...
    let (i, incoming, outgoing) = join_computation(server_url, &format!("{}-offline", room), index)
        .await
    ...
    let (i, incoming, outgoing) = join_computation(server_url, &format!("{}-online", room), index)
        .await
